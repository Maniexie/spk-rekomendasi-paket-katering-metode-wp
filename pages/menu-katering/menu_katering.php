...
$getDataMenuKatering = mysqli_query(
    $koneksi,
    'SELECT * FROM menu_katering
...
$totalDataResult = mysqli_query(
    $koneksi,
    "SELECT COUNT(*) as total FROM menu_katering"
);
...
<section>
    <div class="content">
        <h2 class="content-title text-center">Daftar Menu Katering</h2>
        <div class="container" style="margin-bottom: -20px;">
            <a href="index.php?page=tambah_menu_katering" class="btn btn-primary">+ Tambah Menu</a>
        </div>
        <div class="container mt-4 table-responsive" style="max-height: 700px; overflow-y: auto;">
            <table class="table table-striped table-hover" style="max-height:300px;">
                ...
                <tbody>
                    <?php foreach ($getDataMenuKatering as $row): ?>
                        <tr>
                            <th scope="row"><?= $no++ ?></th>
                            <td><?= $row['nama_menu'] ?></td>
                            </td>
                            <td>
                                <a
                                    href="index.php?page=edit_menu_katering&id_menu_katering=<?= $row['id_menu_katering'] ?>"><i
                                        class="fa-solid fa-pen-to-square"></i></a>
                                <a href="javascript:void(0)"
                                    onclick="konfirmasiDelete('index.php?page=hapus_menu_katering&id_menu_katering=<?= $row['id_menu_katering'] ?>')">
